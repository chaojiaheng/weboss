...
class VideoManagement extends Component {
  constructor(props) {
    ...
    this.columns = [{
      ...
      render: (strName, recordData) => (
        <span>
          <a onClick={() => this.$videoCheck.showModal('view', recordData)}>{strName}</a>
        </span>
      )
    }, {
      title: 'Country',
      ...
      render: (id, record) => (
        <span>
          { record.audit_status === 0 ? <a onClick={() => this.$videoCheck.showModal('check', record)}>Check</a> : <p>Check</p> }
        </span>
      )
    }];
  ...
  getAPIList = (page) => {
    ...
    Utils.request({
      url: `${window.PAY_API_HOST}/baseapi/media/api/video/list`,
      method: 'post',
      headers: {
        token: this.AILA_userInfo ? this.AILA_userInfo.access_token : '',
    ...
    return (
      <Layout>
        <Breadcrumb style={{ margin: '12px 0' }}>
          ...
          <Breadcrumb.Item>Video Management</Breadcrumb.Item>
        </Breadcrumb>
          ...
          <div style={{ width: '100%', padding: '15px' }}>
              ...
              <div className="searchToolbar-item" >
                ...
                <div>
                  <Select
                    showSearch
                    optionFilterProp="children"
                    onChange={value => this.searchOnChange(value, 'audit_status')}
            ...
            <div className="tablecontent">
              <div className="searchToolbar">
                ...
                <div className="searchToolbar-btn">
                  <Button className="rightMargin" type="primary" onClick={() => this.$videoUpload.showModal()} >Upload</Button>
                </div>
              </div>
              <Table
                loading={loading}
            ...
            <VideoCheck onRef={ref => { this.$videoCheck = ref; }} onOK={() => { this.getAPIList(current, pageSize); }} />
            <VideoUpload onRef={ref => { this.$videoUpload = ref; }} onOK={() => { this.getAPIList(1, pageSize); }} />
          </div>
        </Content>
      </Layout>
    );
